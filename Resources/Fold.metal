...
fragment float4 foldFragment(VertexOut in [[stage_in]], texture2d<float> desktop [[texture(0)]], constant Params &p [[buffer(0)]]) {
    ...
    float bend = progress * p.perspective * (1.0f - p.reducedMotion);
    uv.x = 0.5f + (uv.x-0.5f)/(1.0f+0.18f*bend*outer*outer);
    uv.y = 1.0f-pow(max(0.0f,1.0f-uv.y),1.0f+0.42f*bend);
    // Core Image writes Metal textures bottom-up; the drawable is top-down.
    uv.y = 1.0f - uv.y;
    float gradient = pow(outer,1.65f);
    float styleBlur = p.style > 1.5f ? 1.4f : 1.0f;
    float radius = 68.0f * p.blur * pow(progress,1.15f) * gradient * styleBlur * p.width/1600.0f;
    ...
    float shade = p.shade * progress * gradient * (p.style > 0.5f && p.style < 1.5f ? 0.60f : 0.24f);
