...
enum SelfTests {
    static func run() throws {
        ...
        check(!safety.permitsEffect(angle:30,clearAngle:100,now:0),"Partially closed launch fails clear")
        _ = safety.permitsEffect(angle:112,clearAngle:100,now:1)
        _ = safety.permitsEffect(angle:112,clearAngle:100,now:1.3)
        check(safety.permitsEffect(angle:50,clearAngle:100,now:2),"Opening arms next fold")
        check(!safety.permitsEffect(angle:50.1,clearAngle:100,now:4.6) && safety.waitingForOpen,"Held lid and sensor jitter clear after 2.5 seconds")
        check(!safety.permitsEffect(angle:30,clearAngle:100,now:6),"Held-lid timeout stays latched")
        _ = safety.permitsEffect(angle:112,clearAngle:100,now:7)
        _ = safety.permitsEffect(angle:112,clearAngle:100,now:7.3)
        check(safety.permitsEffect(angle:7,clearAngle:100,now:8),"Reopening automatically rearms")
        check(!safety.permitsEffect(angle:6,clearAngle:100,now:8.4),"Nearly closed clears within 350 ms")
        _ = safety.permitsEffect(angle:112,clearAngle:100,now:10)
        _ = safety.permitsEffect(angle:112,clearAngle:100,now:10.3)
        _ = safety.permitsEffect(angle:50,clearAngle:100,now:11)
        check(!safety.permitsEffect(angle:nil,clearAngle:100,now:11.1),"Missing sensor fails clear")
        _ = safety.permitsEffect(angle:112,clearAngle:100,now:12)
        _ = safety.permitsEffect(angle:112,clearAngle:100,now:12.3)
        for i in 0..<80 { _ = safety.permitsEffect(angle:50+sin(Double(i))*10,clearAngle:100,now:13+Double(i)/10) }
        check(!safety.permitsEffect(angle:50,clearAngle:100,now:21.1),"Continuous movement cannot exceed eight seconds")
        safety.suspend()
        check(!safety.permitsEffect(angle:50,clearAngle:100,now:22),"Wake requires reopening")
        let gpu=try FoldGPU()
